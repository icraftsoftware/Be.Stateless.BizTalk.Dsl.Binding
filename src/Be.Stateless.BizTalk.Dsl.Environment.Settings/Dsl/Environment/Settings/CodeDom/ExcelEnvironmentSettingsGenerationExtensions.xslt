...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                xmlns:stringifier="urn:extensions.stateless.be:biztalk:environment-setting-class-generation:stringifier:2021:01"
                xmlns:typifier="urn:extensions.stateless.be:biztalk:environment-setting-class-generation:typifier:2021:01"
    ...
    <xsl:text>
{
    ...
    <xsl:text> : Be.Stateless.BizTalk.Dsl.Environment.Settings.ExcelEnvironmentSettings
	{
</xsl:text>
    <xsl:apply-templates />
    <xsl:text>
		protected override string ExcelFileName => "</xsl:text>
    ...
    <xsl:value-of select="$clr-class-name" />
    <xsl:text> _instance = new </xsl:text>
    <xsl:value-of select="$clr-class-name" />
    <xsl:text>();
  ...
  <xsl:template match="ss:Row[ss:Cell[position()>2]/ss:Data[@ss:Type='Number'][not(typifier:IsInteger(text()))]]/ss:Cell[position()=1][ss:Data/text()]">
    <xsl:call-template name="generate-property-getter">
  ...
  <xsl:template name="generate-property-getter">
    ...
    <xsl:text> => _instance.ValueForTargetEnvironment(new </xsl:text>
    <xsl:value-of select="$type" />
  ...
  <xsl:template match="ss:Cell[ss:Data/@ss:Type='String']" mode="literal">
    <xsl:value-of select="stringifier:Escape(ss:Data/text())" />
  </xsl:template>
