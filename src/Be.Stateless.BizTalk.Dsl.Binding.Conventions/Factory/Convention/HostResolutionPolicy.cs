...
namespace Be.Stateless.BizTalk.Factory.Convention
{
	public class HostResolutionPolicy : Be.Stateless.BizTalk.Dsl.Binding.Convention.HostResolutionPolicy
	{
		...
		{
			return Platform.Settings.ProcessingHost;
		}
		...
		protected override string ResolveHostName<TNamingConvention>(ReceiveLocationTransport<TNamingConvention> transport)
		{
			...
			return transport.Adapter switch {
				HttpAdapter.Inbound => Platform.Settings.IsolatedHost,
				// TODO LogicAppAdapter.Inbound => Platform.Settings.IsolatedHost,
				WcfBasicHttpAdapter.Inbound => Platform.Settings.IsolatedHost,
				WcfWebHttpAdapter.Inbound => Platform.Settings.IsolatedHost,
				WcfWSHttpAdapter.Inbound => Platform.Settings.IsolatedHost,
				var adapter when adapter.GetType().IsSubclassOfGenericType(typeof(WcfCustomIsolatedAdapter.Inbound<>)) => Platform.Settings.IsolatedHost,
				_ => Platform.Settings.ReceivingHost
			};
		}

		protected override string ResolveHostName<TNamingConvention>(SendPortTransport<TNamingConvention> transport)
		{
			return Platform.Settings.TransmittingHost;
		}
