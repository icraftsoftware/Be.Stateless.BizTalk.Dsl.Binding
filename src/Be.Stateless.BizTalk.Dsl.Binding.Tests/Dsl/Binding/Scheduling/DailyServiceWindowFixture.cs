...
namespace Be.Stateless.BizTalk.Dsl.Binding.Scheduling
{
	public class DailyServiceWindowFixture
	{
		...
		{
			Invoking(() => new DailyServiceWindow { Interval = 0 }).Should().Throw<ArgumentOutOfRangeException>();
		}
		...
		{
			Invoking(() => new DailyServiceWindow { Interval = 1000 }).Should().Throw<ArgumentOutOfRangeException>();
		}
	}
}
