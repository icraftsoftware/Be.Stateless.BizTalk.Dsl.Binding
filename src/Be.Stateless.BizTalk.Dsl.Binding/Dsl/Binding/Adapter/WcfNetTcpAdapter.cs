...
namespace Be.Stateless.BizTalk.Dsl.Binding.Adapter
{
		...
		new()
	{
		...
		{
			_protocolType = GetProtocolTypeFromConfigurationClassId(new("7fd2dfcd-6a7b-44f9-8387-29457fd2eaaf"));
		}
		...
		/// <remarks>
		/// <para>
		/// The WCF-NetTcp adapter leverages the <see cref="NetTcpBinding"/> class in the buffered transfer mode to communicate
		/// with an endpoint. For the buffered transport mode, the <see cref="NetTcpBinding"/>.<see
		...
		/// <remarks>
		/// <para>
		/// For more information about the member names for the <see cref="MessageCredentialType"/> property, see the Message
		/// client credential type property in <see
		...
		/// property in <see
		/// href="https://docs.microsoft.com/en-us/biztalk/core/technical-reference/wcf-nettcp-transport-properties-dialog-box-send-security-tab">WCF-NetTcp
		...
		/// from the MessageBox database in a transactional context using the transaction protocol specified in the <see
		/// cref="TransactionProtocol"/> property.
		/// </para>
		/// <para>
		/// For inbound NetTcp adapters, it specifies whether a message is submitted to the MessageBox database using the
		/// transaction flowed from clients. If this property is set to <c>True</c>, the clients are required to submit messages
		/// using the transaction protocol specified in the <see cref="TransactionProtocol"/> property. If the clients submit
