...
namespace Be.Stateless.BizTalk.Unit.Dsl.Binding
{
	public class ApplicationBindingArtifactLookupFactoryFixture
	{
		...
		public void ApplicationName()
		{
			...
			using (new DeploymentContextInjectionScope(targetEnvironment: TargetEnvironment.DEVELOPMENT))
			{
				ApplicationBindingArtifactLookupFactory<TestApplication>.Create().ResolveName().Should().Be("TestApplication");
			}
		}
		...
		public void ReceiveLocationName()
		{
			...
			using (new DeploymentContextInjectionScope(targetEnvironment: TargetEnvironment.DEVELOPMENT))
			{
				ApplicationBindingArtifactLookupFactory<TestApplication>.Create()
					.ReceiveLocation<OneWayReceiveLocation>().ResolveName().Should().Be("OneWayReceiveLocation");
			}
		}
		...
		public void ReceivePortName()
		{
			...
			{
				ApplicationBindingArtifactLookupFactory<TestApplication>.Create()
					.ReceivePort<OneWayReceivePort>().ResolveName().Should().Be("OneWayReceivePort");
			}
		}
		...
		public void ReferencedApplicationName()
		{
			...
			{
				ApplicationBindingArtifactLookupFactory<TestApplication>.Create()
					.ReferencedApplication<TestReferencedApplication>().ResolveName().Should().Be("MyTestReferencedApplication");
			}
		}
		...
		public void ReferencedApplicationNameAddedAtCreationTime()
		{
			...
			using (new DeploymentContextInjectionScope(targetEnvironment: TargetEnvironment.DEVELOPMENT))
			{
				ApplicationBindingArtifactLookupFactory<TestReferencedApplication>.Create(new TestApplication())
					.ReferencedApplication<TestApplication>().ResolveName().Should().Be("TestApplication");
			}
		}
		...
		public void ReferencedApplicationReceiveLocationName()
		{
			...
			using (new DeploymentContextInjectionScope(targetEnvironment: TargetEnvironment.DEVELOPMENT))
			{
				ApplicationBindingArtifactLookupFactory<TestApplication>.Create()
					...
					.ReceiveLocation<TestReferencedReceiveLocation>().ResolveName().Should().Be("MyTestReferencedReceiveLocation");
			}
		}
		...
		public void ReferencedApplicationReceivePortName()
		{
			...
			using (new DeploymentContextInjectionScope(targetEnvironment: TargetEnvironment.DEVELOPMENT))
			{
				ApplicationBindingArtifactLookupFactory<TestApplication>.Create()
					.ReferencedApplication<TestReferencedApplication>()
					.ReceivePort<TestReferencedReceivePort>().ResolveName().Should().Be("MyTestReferencedReceivePort");
			}
		}
		...
		public void ReferencedApplicationSendPortName()
		{
			...
			using (new DeploymentContextInjectionScope(targetEnvironment: TargetEnvironment.DEVELOPMENT))
			{
				ApplicationBindingArtifactLookupFactory<TestApplication>.Create()
					.ReferencedApplication<TestReferencedApplication>()
					.SendPort<TestReferencedSendPort>().ResolveName().Should().Be("MyTestReferencedSendPort");
			}
		}
		...
		public void SendPortName()
		{
			...
			{
				ApplicationBindingArtifactLookupFactory<TestApplication>.Create()
					.SendPort<OneWaySendPort>().ResolveName().Should().Be("OneWaySendPort");
			}
		}
	}
}
