...
{
	/// <summary>
	/// <see cref="IApplicationBindingVisitor"/> decorator that ensures that the BizTalk Server application being deployed has
	/// been settled for its target environment and that only the BizTalk Services belonging to this application are being
	/// visited by the decorated <see cref="IApplicationBindingVisitor"/> implementation.
	/// </summary>
	public class CurrentApplicationBindingVisitor : IApplicationBindingVisitor
	{
		...
		{
			_decoratedVisitor = decoratedVisitor ?? throw new ArgumentNullException(nameof(decoratedVisitor));
		}
