...
{
	/// <summary>
	/// <see cref="IApplicationBindingVisitor"/> base class that ensures that only the BizTalk artifacts belonging to this
	/// application are being visited by the <see cref="MainApplicationBindingVisitor"/>-derived visitor after that the
	/// application binding as well as its referenced applications' bindings have been settled and validated for the target
	/// environment.
	/// </summary>
	public abstract class MainApplicationBindingVisitor : IApplicationBindingVisitor
	{
			...
			where TNamingConvention : class
		{
			...
			((IVisitable<IApplicationBindingVisitor>) applicationBinding).Accept(new EnvironmentOverrideApplicationVisitor());
