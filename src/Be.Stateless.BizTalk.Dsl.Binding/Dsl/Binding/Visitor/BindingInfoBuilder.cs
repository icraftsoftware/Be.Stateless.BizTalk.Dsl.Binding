	...
	public class BindingInfoBuilder : ApplicationBindingVisitor
	{
			...
			where TNamingConvention : class
		{
			var location = new BtsReceiveLocation {
				// General
				Name = receiveLocation.ResolveName(),
				Enable = false, // Enabling Receive Locations is actually the responsibility of BizTalkServiceStateInitializer
				...
				ReceiveHandler = new() {
					Name = receiveLocation.Transport.ResolveHost(),
					TransportType = receiveLocation.Transport.Adapter.ProtocolType
				},
			...
			where TNamingConvention : class
		{
			var port = new BtsSendPort {
				...
				// sendPort.Status is the responsibility of BizTalkServiceStateInitializer
