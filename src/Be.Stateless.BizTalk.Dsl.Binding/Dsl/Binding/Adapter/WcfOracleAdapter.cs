...
namespace Be.Stateless.BizTalk.Dsl.Binding.Adapter
{
		...
		new()
	{
		...
		/// procedures. Default is <c>True</c>, which means the adapter will skip passing values for nodes that are marked as
		/// 'nil'. In this case, the default value in Oracle (if specified) is taken into account for nodes that are marked as
		/// 'nil'. If set to <c>False</c>, the adapter explicitly passes a null value for these nodes.
		/// </para>
		/// <para>
		/// Notice that:
		/// <list type="bullet">
		...
		/// <remarks>
		/// <para>
		/// The default value is <c>True</c>, which means that the adapter always performs the operations in a transaction
		/// context, assuming that the client is providing the transactional context. If there are other resources participating
		/// in the transaction, the connections created enlist in <see cref="Transaction"/> and are elevated to an MSDTC
		/// transaction.
		/// </para>
		/// <para>
		...
		/// MSDTC transaction can be a performance overhead. In such scenarios, you can set the binding property to <c>False</c>
		...
		/// cref="OracleDBBindingConfigurationElement.GeneratedUserTypesAssemblyFilePath"/> property while generating metadata.
		...
		/// <remarks>
		/// <para>
		/// The default is one. For values of <see cref="InsertBatchSize"/> greater than one, the Oracle Database adapter batches
		...
		/// insert message has 10 records and the <see cref="InsertBatchSize"/> is set to 1, the adapter reads individual records
		/// and writes them into the Oracle database. So, the adapter performs 10 separate operations on the Oracle database.
		/// Similarly, if the insert message has 10 records and the <see cref="InsertBatchSize"/> is set to 5, the adapter will
		/// read and write 5 records at a time into the Oracle database, therefore performing only 2 insert operations.
		/// </para>
		/// <para>
		/// If the structure of the records is not the same across a batch, a <see cref="XmlReaderParsingException"/> exception
		/// is thrown and the transaction is rolled back for the entire insert operation. A well-chosen value for <see
		/// cref="InsertBatchSize"/> can greatly improve adapter performance for multiple record Insert operations.
		/// </para>
		/// </remarks>
...
#pragma warning restore 618
		...
		/// <see href="https://docs.microsoft.com/en-us/biztalk/adapters-and-accelerators/adapter-oracle-database/basic-oracle-data-types1">Basic Oracle Data Types</see>.
		/// </para>
		/// <para>
		...
		/// the SCOTT.EMP table is "<c>http://Microsoft.LobServices.OracleDB/2007/03/SCOTT/Table/EMP</c>"; if it is <c>False</c>,
		/// the namespace is "<c>http://Microsoft.LobServices.OracleDB/2007/03/Table/EMP</c>".
		/// </para>
		/// <para>
		/// The message action is not affected by the <see cref="UseSchemaInNameSpace"/> binding property; it always includes the
		/// schema name.
		/// </para>
		/// <para>
		/// </para>
		/// It is strongly recommended to set this binding property to <c>True</c> while generating metadata. If you set this
		/// property to <c>False</c>, the Oracle schema names (for example, SCOTT) will not be available in the XML namespace of
		...
		/// <remarks>
		/// <para>
		/// This is an ODP.NET property that is used for performance tuning. You must set MaxPoolSize judiciously. It is possible
		/// to exhaust the number of connections available from ODP.NET, if this value is set too large.
		/// </para>
		/// <para>
		/// The default is 100.
		/// </para>
